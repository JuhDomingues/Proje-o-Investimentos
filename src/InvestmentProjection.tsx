...
import { TrendingUp, DollarSign, Target, Users, Zap, Save, Database } from 'lucide-react';
import { useCampaign } from './hooks/useCampaign';

const InvestmentProjection = () => {
  // Hook para gerenciar dados da campanha
  const { campaign, loading, error, updateCPA, updateWeek, reload } = useCampaign('Outubro', 2024);

  // Estado local para CPA (sincronizado com o banco)
  const [cpa, setCpa] = useState(100);
  const [isSaving, setIsSaving] = useState(false);

  // Sincronizar CPA com a campanha carregada
  useEffect(() => {
    if (campaign) {
      setCpa(campaign.cpa);
    }
  ...
  // Função para salvar CPA com debounce
  const handleCPAChange = async (newCpa: number) => {
    setCpa(newCpa);
    if (campaign) {
      setIsSaving(true);
      await updateCPA(newCpa);
      setIsSaving(false);
    }
  };

  // Função para atualizar dados de uma semana
  const handleWeekUpdate = async (weekNumber: number, field: string, value: number) => {
    if (!campaign) return;

    setIsSaving(true);
    await updateWeek(weekNumber, { [field]: value });
    setIsSaving(false);
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600">Carregando dados da campanha...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-lg shadow-lg">
          <p className="text-lg text-red-600 mb-4">Erro ao carregar campanha: {error}</p>
          <button
            onClick={reload}
  ...
  // Obter dados das semanas
  const weeks = campaign.weeks.sort((a, b) => a.weekNumber - b.weekNumber);

  const [week1, week2, week3, week4] = weeks;

  // Estados locais para agendamentos e vendas gerais
  const [agendamentos, setAgendamentos] = useState(
    weeks.reduce((sum, w) => sum + w.agendamentos, 0)
  );
  const [comparecimentos, setComparecimentos] = useState(
    weeks.reduce((sum, w) => sum + w.comparecimentosReais, 0)
  );
  const [vendasMentoria, setVendasMentoria] = useState(
    weeks.reduce((sum, w) => sum + w.vendasMentoria, 0)
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">🎛️ Simulador de CPA</h2>
            {isSaving && (
          ...
          <div className="mb-6">
            ...
            <input
              ...
              max="150"
              value={cpa}
              onChange={(e) => handleCPAChange(Number(e.target.value))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {weeks.map((week) => (
              <div key={week.weekNumber}>
                ...
                <input
                  type="number"
                  value={week.comparecimentosReais}
                  onChange={(e) => handleWeekUpdate(week.weekNumber, 'comparecimentosReais', Number(e.target.value))}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="0"
                />
                <p className="text-xs text-gray-500 mt-1">Meta: {week.metaComparecimentos}</p>
              </div>
            ))}
          </div>
              ...
              <tbody>
                <tr className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week1.vendasRealizadas}
                      onChange={(e) => handleWeekUpdate(1, 'vendasRealizadas', Number(e.target.value))}
                  ...
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week1.investimentoReal}
                      onChange={(e) => handleWeekUpdate(1, 'investimentoReal', Number(e.target.value))}
                      className="w-24 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4 text-blue-600 font-semibold">
                    {week1.cpaReal ? formatCurrency(week1.cpaReal) : '-'}
                  </td>
                ...
                <tr className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week2.vendasRealizadas}
                      onChange={(e) => handleWeekUpdate(2, 'vendasRealizadas', Number(e.target.value))}
                      className="w-20 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week2.investimentoReal}
                      onChange={(e) => handleWeekUpdate(2, 'investimentoReal', Number(e.target.value))}
                      className="w-24 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4 text-blue-600 font-semibold">
                    {week2.cpaReal ? formatCurrency(week2.cpaReal) : '-'}
                  </td>
                  <td className="text-center py-4 px-4">
                ...
                <tr className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week3.vendasRealizadas}
                      onChange={(e) => handleWeekUpdate(3, 'vendasRealizadas', Number(e.target.value))}
                      className="w-20 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week3.investimentoReal}
                      onChange={(e) => handleWeekUpdate(3, 'investimentoReal', Number(e.target.value))}
                      className="w-24 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4 text-blue-600 font-semibold">
                    {week3.cpaReal ? formatCurrency(week3.cpaReal) : '-'}
                  </td>
                  <td className="text-center py-4 px-4">
                ...
                <tr className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week4.vendasRealizadas}
                      onChange={(e) => handleWeekUpdate(4, 'vendasRealizadas', Number(e.target.value))}
                      className="w-20 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4">
                    <input
                      type="number"
                      value={week4.investimentoReal}
                      onChange={(e) => handleWeekUpdate(4, 'investimentoReal', Number(e.target.value))}
                      className="w-24 px-2 py-1 border border-gray-300 rounded text-center"
                      placeholder="0"
                    />
                  </td>
                  <td className="text-center py-4 px-4 text-blue-600 font-semibold">
                    {week4.cpaReal ? formatCurrency(week4.cpaReal) : '-'}
                  </td>
                  <td className="text-center py-4 px-4">
